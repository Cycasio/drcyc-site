...
```
你符合 BMI 適應症（≥ 30，或 ≥ 27 + 共病）?
├── 否 → 不建議用藥，請先做生活型態改變
└── 是 → 繼續

  你有 GLP-1 禁忌症（MTC 家族史、胰臟炎史、孕婦）?
  ├── 是 → 不能用 GLP-1，討論其他減重藥物（Contrave、Orlistat）
  └── 否 → 繼續

    預算考量 + 目標減重幅度?
    ├── 預算寬鬆 + 目標 > 15% → Mounjaro（最強）
    ├── 預算中等 + 目標 10-15% → Wegovy 或 Mounjaro 低劑量
    ├── 預算有限 + 想試試 → Saxenda（每日注射，但較便宜）
    └── 怕針 → Rybelsus（口服，但效果偏弱、空腹規定嚴）

      合併第二型糖尿病?
      └── 可考慮健保適應症的 Ozempic / Mounjaro 部分劑量 / Trulicity
```
